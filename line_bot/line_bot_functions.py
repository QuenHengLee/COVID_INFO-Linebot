...
def news_subscribe_flex(event): # 呼叫 訂閱新聞的清單(FLEX MSG)
    try:
        ...
        FlexMessage = news_card.subscribe_news_flex_card(have_subscribed_news)                
...
def situation_subscribe_flex(event): # 呼叫 訂閱現況的清單(FLEX MSG)
    try:
        ...
        FlexMessage = news_card.subscribe_situation_flex_card(have_subscribed_situation)                
    ...
    try:
        ...
        quick_reply=QuickReply(
            items=[QuickReplyButton(action=MessageAction(
                    ...
                    QuickReplyButton(action=MessageAction(
                        label="中國", text="中國最新現況", data='country=china')),
                    ]))
        line_bot_api.reply_message(event.reply_token, message)
    except Exception as e:
        line_bot_api.reply_message(
    ...
    try:
        ...
        quick_reply=QuickReply(
            items=[                                                                 # 對應到showTokyoSituation
                    ...
                    QuickReplyButton(action=MessageAction(
                        label="中國", text="中國入境規定", data='country=china')),
                    ]))
        line_bot_api.reply_message(event.reply_token, message)
    except Exception as e:
        error_process(e,event)
...
def trans_m4a_to_wav(filepath, out_name):       #把line接接收到的audio由m4a轉檔為wva，trans_m4a_to_wav(檔案路徑, 輸出檔案名稱)
    # print("我還活著")
    # pydub.AudioSegment.converter = '../ffmpeg-N-104005-g2761a7403b-win64-gpl/bin/ffmpeg.exe'
